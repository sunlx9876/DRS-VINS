...
ProjectionOneFrameTwoCamFactor::ProjectionOneFrameTwoCamFactor(const Eigen::Vector3d &_pts_i, const Eigen::Vector3d &_pts_j,
                                                               ...
                                                               double _dynamic_weight, double _structure_weight) : 
                                                               pts_i(_pts_i), pts_j(_pts_j), 
                                                               td_i(_td_i), td_j(_td_j),
                                                               dynamic_weight(_dynamic_weight),
                                                               structure_weight(_structure_weight),
                                                               fused_weight(std::max(MIN_FUSED_WEIGHT, _dynamic_weight * _structure_weight))
{
...
{
    TicToc tic_toc;
    const double weight_scale = std::sqrt(std::max(MIN_FUSED_WEIGHT, fused_weight));
    const Eigen::Matrix2d weighted_sqrt_info = weight_scale * sqrt_info;
    ...
    residual = weighted_sqrt_info * residual;
    ...
    if (jacobians)
    {
...
#endif
        reduce = weighted_sqrt_info * reduce;
        ...
        if (jacobians[3])
        {
            ...
            jacobian_td = reduce * ric2.transpose() * ric * velocity_i / inv_dep_i * -1.0  +
                          weighted_sqrt_info * velocity_j.head(2);
        }
    }
    sum_t += tic_toc.toc();
...
#endif
        tmp_residual = weighted_sqrt_info * tmp_residual;
