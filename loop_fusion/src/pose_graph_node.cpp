...
int ROW;
int COL;
double FOCAL_LENGTH_X = 460.0;
double FOCAL_LENGTH_Y = 460.0;
...
int main(int argc, char **argv)
{
    ...
    if(argc != 2)
    {
        printf("please intput: rosrun loop_fusion loop_fusion_node [config file] \n"
               ...
               "~/catkin_ws/src/DRS-VINS/config/realsense_d435i/realsense_stereo_imu_config.yaml \n");
        return 0;
    }
    ...
    if (!readCameraIntrinsics(cam0Path, 460.0, 460.0, COL / 2.0, ROW / 2.0,
                              FOCAL_LENGTH_X, FOCAL_LENGTH_Y, PRINCIPAL_POINT_X, PRINCIPAL_POINT_Y))
    {
        ROS_WARN_STREAM("Failed to read loop fusion camera intrinsics from " << cam0Path << ", fallback to default intrinsics");
        FOCAL_LENGTH_X = 460.0;
        FOCAL_LENGTH_Y = 460.0;
        PRINCIPAL_POINT_X = COL / 2.0;
        PRINCIPAL_POINT_Y = ROW / 2.0;
    }
    ...
    createDirectories(POSE_GRAPH_SAVE_PATH);
    createDirectories(VINS_RESULT_PATH);
    ...
    ros::Subscriber sub_vio = n.subscribe("/drs_estimator/odometry", 2000, vio_callback);
    ros::Subscriber sub_image = n.subscribe(IMAGE_TOPIC, 2000, image_callback);
    ros::Subscriber sub_pose = n.subscribe("/drs_estimator/keyframe_pose", 2000, pose_callback);
    ros::Subscriber sub_extrinsic = n.subscribe("/drs_estimator/extrinsic", 2000, extrinsic_callback);
    ros::Subscriber sub_point = n.subscribe("/drs_estimator/keyframe_point", 2000, point_callback);
    ros::Subscriber sub_margin_point = n.subscribe("/drs_estimator/margin_cloud", 2000, margin_point_callback);
