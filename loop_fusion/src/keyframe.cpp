                                      ...
                                      vector<uchar> &status)
{
    ...
    if (n >= 8)
    {
        ...
        {
            double tmp_x, tmp_y;
            tmp_x = FOCAL_LENGTH_X * matched_2d_cur_norm[i].x + PRINCIPAL_POINT_X;
            tmp_y = FOCAL_LENGTH_Y * matched_2d_cur_norm[i].y + PRINCIPAL_POINT_Y;
            tmp_cur[i] = cv::Point2f(tmp_x, tmp_y);

            tmp_x = FOCAL_LENGTH_X * matched_2d_old_norm[i].x + PRINCIPAL_POINT_X;
            tmp_y = FOCAL_LENGTH_Y * matched_2d_old_norm[i].y + PRINCIPAL_POINT_Y;
                         ...
                         Eigen::Vector3d &PnP_T_old, Eigen::Matrix3d &PnP_R_old)
{
    ...
    cv::Mat inliers;
    TicToc t_pnp_ransac;
    const double focal_scale = std::max(FOCAL_LENGTH_X, 1.0);
    const double normalized_threshold = 10.0 / focal_scale;

    if (CV_MAJOR_VERSION < 3)
        solvePnPRansac(matched_3d, matched_2d_old_norm, K, D, rvec, t, true, 100, normalized_threshold, 100, inliers);
    else
    {
        if (CV_MINOR_VERSION < 2)
            solvePnPRansac(matched_3d, matched_2d_old_norm, K, D, rvec, t, true, 100, sqrt(normalized_threshold), 0.99, inliers);
        else
            solvePnPRansac(matched_3d, matched_2d_old_norm, K, D, rvec, t, true, 100, normalized_threshold, 0.99, inliers);
